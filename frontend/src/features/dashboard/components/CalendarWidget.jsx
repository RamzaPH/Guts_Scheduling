...
const LOCAL_PH_HOLIDAYS_BY_YEAR = {
  2026: {
    ...
    "2026-03-20": "Eid'l Fitr (Estimated)",
    "2026-04-02": "Maundy Thursday",
    "2026-04-03": "Good Friday",
    "2026-04-04": "Black Saturday",
    "2026-04-09": "Day of Valor",
    "2026-05-01": "Labor Day",
    "2026-05-11": "National Election Day",
    "2026-05-27": "Eid'l Adha (Estimated)",
...
const ENGLISH_HOLIDAY_LABELS = {
  ...LOCAL_PH_HOLIDAYS_BY_YEAR[2026]
};
...
function isDateAllowedForCourse(date, courseFilter) {
  ...
  if (normalized === "tdc") return day !== 0; 
  if (normalized === "pdc" || normalized === "pdc_beginner" || normalized === "pdc_experience") return day >= 1 && day <= 6; 
  return true;
}
...
function normalizeHolidayResponse(items) {
  const holidayMap = {};
  if (!Array.isArray(items)) return holidayMap;
...
export default function CalendarWidget({
  view, courseFilter = "tdc", onPrevMonth, onNextMonth, reportFilter, onSelectDate, onOpenSchedule, monthStatus = [],
}) {
  ...
  useEffect(() => {
    ...
    async function loadPhilippineHolidays() {
      try {
        const response = await fetch(`https://date.nager.at/api/v3/PublicHolidays/${view.year}/PH`, { signal: controller.signal });
        if (!response.ok) return;
        const payload = await response.json();
        const remoteMap = normalizeHolidayResponse(payload);
        if (Object.keys(remoteMap).length === 0) return;
  ...
  // ✅ FIX: Binalot na natin sa useMemo para hindi mag-warning ang ESLint
  const holidayMap = useMemo(() => ({
    ...localHolidayMap,
    ...(remoteHolidayData?.year === view.year ? remoteHolidayData.map : {})
  }), [localHolidayMap, remoteHolidayData, view.year]);
  
  const holidaySource = remoteHolidayData?.year === view.year ? "Merged API & Local" : "Local fallback list";
    ...
    <div className="rounded-xl border-t-2 border-t-[#D4AF37] border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid grid-cols-7 gap-1 text-center text-xs text-slate-500">
        ...
        {cells.map((day, index) => {
          ...
          const hasClass = monthStatusMap.has(isoDate); 
          ...
          return (
            <button
              ...
              type="button"
              disabled={!isAllowed}
              onClick={() => { if (isAllowed) onSelectDate(current); }}
              onDoubleClick={() => { if (isAllowed) { onSelectDate(current); onOpenSchedule(current); } }}
              ...
              className={`relative h-9 rounded-md text-sm font-semibold ${
                !isAllowed
                  ...
                  : hasClass
                    ? "cursor-pointer border border-amber-300 bg-amber-50 text-amber-800 hover:bg-amber-100"
                  : "cursor-pointer text-slate-700 hover:bg-slate-100"
              } ${isPast && !isSelected ? "opacity-55" : ""}`}
            >
              {day}
              {isHoliday && <span className="absolute left-0.5 top-0.5 rounded-full bg-emerald-600 px-1 py-0.5 text-[8px] font-bold uppercase tracking-[0.1em] text-white">Hol</span>}
              {isFull && <span className="absolute right-0.5 top-0.5 rounded-full bg-red-600 px-1 py-0.5 text-[8px] font-bold uppercase tracking-[0.16em] text-white">Full</span>}
              {hasClass && !isHoliday && !isFull && <span className="absolute bottom-1 left-1/2 h-1.5 w-5 -translate-x-1/2 rounded-full bg-amber-500/70" />}
            </button>
          );
        })}
      </div>
      ...
      <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
        ...
        <p className="mt-0.5 text-[10px] uppercase tracking-wide text-emerald-700/80">Source: {holidaySource}</p>
        <p className="mt-1 text-[10px] font-medium text-emerald-700/90">Legal holidays are clickable for scheduling.</p>
        {monthHolidayEntries.length > 0 ? (
          <ul className="mt-1 space-y-0.5">
            {monthHolidayEntries.map(([isoDate, label]) => <li key={isoDate}>{isoDate}: {label}</li>)}
          </ul>
        ) : (
